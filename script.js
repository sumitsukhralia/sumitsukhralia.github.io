...
// This block runs immediately once the DOM is fully loaded.
document.addEventListener('DOMContentLoaded', function() {
    ...
    function showNextGreeting() {
        if (currentGreetingIndex > 0) {
            const prevElement = document.getElementById(greetings[currentGreetingIndex - 1]);
            if (prevElement) {
                prevElement.style.opacity = '0'; // Fade out previous greeting
            }
        }

        if (currentGreetingIndex < greetings.length) {
            const currentElement = document.getElementById(greetings[currentGreetingIndex]);
            if (currentElement) {
                currentElement.style.opacity = '1'; // Fade in current greeting
            }
            currentGreetingIndex++;
            setTimeout(showNextGreeting, 120); // Display next greeting after 120ms
        } else {
            // All greetings have been displayed, now fade out the preloader
            setTimeout(() => {
                if (preloader) {
                    ...
                    }, 500); // Match CSS transition for preloader fade
                }
            }, 500); // Short delay before preloader fade out (allows last greeting to be seen)
        }
    }
    ...
    // The rest of the main script logic will be placed here
    // to ensure it runs AFTER the preloader sequence has initiated.
});


// --- Dark Mode Toggle Functionality (Called by button click) ---
// This function needs to be globally accessible (not inside DOMContentLoaded)
// because it's called directly by an `onclick` attribute in your HTML.
let userPrefersDarkMode = false; // Global flag to store user's manual preference
...
// --- Main Website Functionality (runs AFTER DOM is fully loaded and preloader sequence has started) ---
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Correctly targets .terminal-content now, as per your HTML structure.
            ...
            animationInterval = setInterval(() => {
                terminal.textContent = originalText
                    ...
                    .map((char, index) => {
                        ...
                        // Generate a random uppercase letter (ASCII 'A' is 65)
    ...
    function displayNextQuote() {
        ...
        if (quotes.length === 0) { // Handle empty quotes array
        ...
        quoteBox.style.opacity = 0; // Fade out current quote
        console.log(`Displaying quote #${currentQuoteIndex}: "${quotes[currentQuoteIndex].substring(0, 30)}..."`);

        setTimeout(() => {
            quoteBox.innerText = quotes[currentQuoteIndex]; // Set new quote text
    ...
    if (quotes.length > 0) {
        displayNextQuote(); // Display first quote on load
    ...
    if (newQuoteBtn) {
        newQuoteBtn.addEventListener('click', () => {
            ...
            displayNextQuote(); // Display a new quote immediately
    ...
    let hasScrolledPastTerminal = false; // This variable needs to be scoped correctly
    ...
    if (terminalElement) {
        ...
        window.addEventListener('scroll', () => {
            ...
            if (terminalBottom < 0 && !hasScrolledPastTerminal) {
                document.body.classList.add('dark-mode');
                // Update dark mode button icon when auto-switching
                const icon = document.querySelector('.toggle-dark i');
                if (icon) {
                    icon.classList.remove('fa-moon');
                    icon.classList.add('fa-sun');
                ...
                if (userPrefersDarkMode) { // Re-apply if user manually set dark mode
                    document.body.classList.add('dark-mode');
                    const icon = document.querySelector('.toggle-dark i');
                    if (icon) {
                        icon.classList.remove('fa-moon');
                ...
                } else { // Ensure light mode icon if user preference is light
                    const icon = document.querySelector('.toggle-dark i');
                    if (icon) {
                        icon.classList.remove('fa-sun');
                        icon.classList.add('fa-moon');
